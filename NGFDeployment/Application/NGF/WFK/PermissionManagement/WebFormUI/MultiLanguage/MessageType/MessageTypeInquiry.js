...
var $textTypeName;
var $textTypeCode;
var $textTypeDescription;

$(function () {
    $selectCategory = $("#selectCategory");
    $textTypeName = $("#textTypeName");
    $textTypeCode = $("#textTypeCode");
    $textTypeDescription = $("#textTypeDescription");
...
function buttonNew_click() {
    openMessageTypeMaintain("", pageAction.New);
    return;
}

function datagrid1Edit() {
    var baseMessageTypeDTO = $("#datagrid1").datagrid("getSelected");
    if (baseMessageTypeDTO == null) {
        //未选中行，不处理
        return;
    }
    openMessageTypeMaintain(baseMessageTypeDTO.Id, pageAction.Edit);

    return;
}

function openMessageTypeMaintain(id, action) {
    $.QDialog.show(
        {
            title: 'Message Type Maintain'
        },
        {
            url: "MessageTypeMaintain.aspx?" + $.param({ PageAction: action, id: id }),
            width: 0.7,
            height: 0.4,
    ...
    $.messager.confirm("Confirm", "Are you sure to delete?",
        function (confirmresult) {
            if (confirmresult == true) {
                $.qajax({
                    url: "MessageTypeService.asmx/DeleteBaseMessageType",
                    data: $.toJSON({ messageTypeId: baseMessageTypeDTO.Id }),
...
function getQueryUrl() {
    var vArguments = {
        ...
        TypeName: encodeURI($.trim($textTypeName.val())),
        TypeCode: encodeURI($.trim($textTypeCode.val())),
        TypeDescription: encodeURI($.trim($textTypeDescription.val()))
    };

    var queryurl = "GetBaseMessageTypeList.ashx?" + $.param(vArguments);
    return queryurl;
}
...
function buttonInquiry_click() {
    ...
    $('#datagrid1').datagrid({
        columns: [[
                    { field: 'Category', title: 'Category', width: 100, halign: 'center' },
                    { field: 'Type_Name', title: 'Type', width: 100, halign: 'center' },
                    { field: 'Type_Code', title: 'Type Code', width: 100, halign: 'center' },
                    { field: 'Type_Description', title: 'Description', width: 250, halign: 'center' }
                 ]],
