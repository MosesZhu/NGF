...
function buttonNew_click() {
    openProductMaintain("", pageAction.New);
    return;
}

function datagrid1Edit() {
    var baseProductDTO = $("#datagrid1").datagrid("getSelected");
    if (baseProductDTO == null) {
        //未选中行，不处理
        return;
    }
    openProductMaintain(baseProductDTO.Id, pageAction.Edit);

    return;
}

function openProductMaintain(id, action) {
    $.QDialog.show(
        {
        ...
        },
        {
            width: 0.9,
            height: 0.9,
            url: "ProductMaintain.aspx?" + $.param({ PageAction: action, id: id }),
            onCloseCallback: function (returnValue) {
                if (returnValue == true) {
...
function datagrid1Delete() {
    ...
    $.messager.confirm("Confirm", "Are you sure to delete?", function (confirmresult) {
        if (confirmresult == true) {
            $.qajax({
                url: "ProductService.asmx/DeleteBaseProduct",
                data: $.toJSON({ id: baseProductDTO.Id }),
...
function getQueryUrl() {
    var vArguments = {
        ...
        ImpersonateAdminRole: encodeURI($.trim($textImpersonateAdminRole.val()))
    };

    var queryurl = "GetBaseProductList.ashx?" + $.param(vArguments);
    return queryurl;
}
...
function buttonInquiry_click() {
    ...
    $('#datagrid1').datagrid({
        columns: [[
                    ...
                    { field: 'ImpersonateAdminRole', title: 'Impersonate Admin Role', width: 300, halign: 'center' }
                ]],
...
function buttonImport_click() {
    $.QDialog.show(
        ...
        },
        {
            width: 0.8,
            height: 0.3,
            url: "../../CommonPage/FileUpload.aspx?function=Product",
            onCloseCallback: function (returnValue) {
                var fileName = $.trim(returnValue);
                if (fileName != "") {
                    openProductImport(fileName);
                }
            }
        }
    );
...
function openProductImport(fileName) {
    $.QDialog.show(
        {
        ...
        },
        {
            width: 0.9,
            height: 0.9,
            url: "ProductImport.aspx?" + $.param({ fileName: fileName }),
            onCloseCallback: function (returnValue) {
                if (returnValue == true) {
                    $('#datagrid1').datagrid("reload");
                    $('#datagrid1').datagrid("unselectAll"); //取消选择所有页面的行。
...
function btnExport_click() {
    var listBaseProductDTO = $("#datagrid1").datagrid("getSelections");
    if (listBaseProductDTO.length == 0) {
        $.messager.alert("Warning", "Please select data first!", "warning");
        return;
    }
    $(listBaseProductDTO).each(function (i, data) {
    ...
    $.qajax({
        url: "ProductService.asmx/GetProductXml",
